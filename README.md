...
| `production-prompts/MASTER-PROMPT-sports.md` | **Sports-head drama master prompt (V16.3, real-team)** — for the anthropomorphic sports-ball-head niche (see `reference/sports-head-drama-breakdown.md`). 6 phases: **single-image** Asset Bank (all 9:16, no split-screen/turnaround), **9:16 single-frame Image Prompts** (anti-split-screen lock — multi-shot/"shot 1→2" language banned from image prompts), **GROK** multi-shot motion, **KLING** condensed motion (≤2500), **Veo Omni with references** (native audio, ≤7 @assets, no priming image), + **bilingual EN/VI CapCut breakdown with a master shot table** mapping every scene to its image file. Slow-storytelling **~62 WPM**, real NFL/NBA/MLB/NHL/MLS teams + exact hex |
...
| `production-prompts/gem-setup-1-topic.md` · `gem-setup-2-script.md` · `gem-setup-3-render.md` | **Ready-to-paste Google Gemini "Gem" setups** for the sports-head pipeline — one Gem each for Topic / Script / Render. Each file gives the Gem config (name, model, knowledge files, search setting) + a copy-paste **Operating Header** (enforces phase-by-phase "Continue", the locks, engine tags, no-real-athletes) that wraps the matching skill / master prompt |
| `production-prompts/gem-setup-1-topic-hook.md` · `gem-setup-2-script-hook.md` | **Gem setups for the self-contained `*-hook` skills** (use with `gem-setup-3-render.md`). Same as above but for `topic-sports-hook` / `script-sports-hook` — since those skills embed the VIRAL HOOK + INSULT BANK, no external bank file is needed; the Operating Headers enforce the **0-3s HOOK INSULT LOCK** (every brief carries a hook insult; every script opens Shot 1 on it) + the X1-X11 anti-duplication rule |
...
| `.kiro/skills/topic-sports-hook/SKILL.md` · `.kiro/skills/script-sports-hook/SKILL.md` | **Self-contained "viral-hook" variants** of the sports topic + script skills — the VIRAL HOOK + INSULT BANK (0-3s "hook = insult" formula, trash-talk line bank, visual-hook recipe, ⛔ banned-premise list X1-X11) is **embedded inside each skill** (no external knowledge file needed), so they're ready to paste straight into a Gem. Every brief locks a **HOOK INSULT LINE** and every script **opens Shot 1 on that insult**. Same tri-stream + real-team + ~62 WPM + V16.3 handoff as the base pair |
